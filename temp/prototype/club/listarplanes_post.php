<?php
      ...
      function buscar($b) {
            ...
            $sql = mysql_query("select id, nsocio, foto, nombre, apellido, tipo from socios WHERE (nombre LIKE '%".$b."%' OR apellido LIKE '%".$b."%' OR nsocio LIKE '%".$b."%') and (estado = 'Activo') " ,$con);
			...
			<tr>
				...
				<th class="success text-center">Carnet</th>
			</tr>
			<?php
				...
				<tr>
					<td class="text-center">
						<img class="img-rounded" src="foto\socio\<?php echo $vfoto;?>" width="75px" height="75px">
					</td>
					...
					<?php
					if ($row['tipo'] == 'Civil'){
						echo '<h7><span class="label label-primary text-center"><a href="modificarformularioscivil.php?accion=modifica&idu='.$row['id'].'&close=dor"><font color=white><span class="glyphicon glyphicon-pencil" aria-hidden="true"></a></font></span></h7>';
					}else{
						if ($row['tipo'] == 'Pensionista'){
							echo '<h7><span class="label label-primary text-center"><a href="modificarformulariospensionista.php?accion=modifica&idu='.$row['id'].'&close=dor"><font color=white><span class="glyphicon glyphicon-pencil" aria-hidden="true"></a></font></span></h7>';
						}else{
							if ($row['tipo'] == 'Militar'){
								echo '<h7><span class="label label-primary text-center"><a href="modificarformulariosmilitar.php?accion=modifica&idu='.$row['id'].'&close=dor"><font color=white><span class="glyphicon glyphicon-pencil" aria-hidden="true"></a></font></span></h7>';
							}
						}
					}
					?>
					<span class="label label-warning text-center">
					<a href="fotoperfil.php?accion=modifica&idf=<?php echo $row['id'];?>&close=dor">
				...
				<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
				  <div class="modal-dialog" role="document">
					<div class="modal-content">
					  <div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h4 class="modal-title" id="myModalLabel"><span class="label label-success">Concepto</span></h4>
					  </div>
					  <div class="modal-body">
  ...
  function consultar() { 
  	 $con = mysql_connect('localhost','root', '');
            ...
            $sql = mysql_query("select id, nsocio, foto, nombre, apellido, tipo from socios where estado = 'Activo' " ,$con);
            ...
            if($b = trim("")){
				...
				<tr>
					...
					<?php
					if ($row['tipo'] == 'Civil'){
						echo '<h7><span class="label label-primary text-center"><a href="modificarformularioscivil.php?accion=modifica&idu='.$row['id'].'&close=dor"><font color=white><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></font></a></span></h7>';
					}else{
						if ($row['tipo'] == 'Pensionista'){
							echo '<h7><span class="label label-primary text-center"><a href="modificarformulariospensionista.php?accion=modifica&idu='.$row['id'].'&close=dor"><font color=white><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></font></a></span></h7>';
						}else{
							if ($row['tipo'] == 'Militar'){
								echo '<h7><span class="label label-primary text-center"><a href="modificarformulariosmilitar.php?accion=modifica&idu='.$row['id'].'&close=dor"><font color=white><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></font></a></span></h7>';
							}
						}
					}
					?>
					...
					<td class="text-center">
					<span class="label label-success text-center">
					<a href="#" data-toggle="modal" data-target="#myModal3" data-id="<?php echo $row['id'];?>">
					<font color=white><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></font></a>
					</span>
					</td>
					<td class="text-center">
					<span class="label label-default text-center">
					<a href="#" data-toggle="modal" data-target="#myModal4" data-id="<?php echo $row['id'];?>">
					<font color=white><span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span></font></a>
					</span>
					</td>
				</tr>
...
<script type="text/javascript">
    ...
    {   
		console.log($(this).attr('data-id'));	
			...
			$.ajax({
                    url: 'buscartablacarnet.php?id='+$(this).attr("data-id"),
                    type    : 'POST',
                    dataType: 'json',
                    success: 
                        function( data ) {
                          
                           options = '<table class="table table-striped table-bordered"><tr><th class="success">Id carnet</th><th class="success">Nombre plan</th><th class="success">Cuotas pagas</th><th class="success">Cant. cuotas</th><th class="success">Vencimiento</th><th class="success">Imprimir</th></tr>';
                                for(var i=0;i<data.length; i++)
                            {
                                options = options+'<tr><td class="text-center">'+data[i].idplan+'</td><td>'+data[i].descripcion+'</td><td class="text-right">'+data[i].cuotas+'</td><td class="text-right">'+data[i].pagadas+'</td><td class="text-right">'+data[i].vencimiento+'</td><td class="text-right"><span class="label label-danger text-center"><a href="buscarimprimircarnet.php?accion=imprimir&idf='+data[i].idplan+'&close=dor" target="_blank"><font color=white><span class="glyphicon glyphicon-credit-card" aria-hidden="true"></span></font></a></span></td></tr>';    
