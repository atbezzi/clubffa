<?php
	...
	$idu = $_REQUEST['idu'];
	$queryusuario="SELECT p.importetotal as importetotal, p.socio_id as socio_id, s.nsocio as numsocio, s.nombre as nombre, 
	s.apellido as apellido FROM pagos p inner join socios s on p.socio_id = s.id WHERE s.nsocio = '$idu'";
									...
									<div class="table-responsive">
										<table class="table table-striped table-bordered">
											...
											if ($accion =="imprimir")
													...
													$otrosql="select sum(c.importe) as total from cuotas c inner join plan_socio ps on ps.id = c.plan_socio_id 
													inner join socios s on s.id = ps.socio_id 
													where c.estado = 'Pendiente' and EXTRACT(YEAR_MONTH FROM c.vencimiento) <= EXTRACT(YEAR_MONTH FROM now()) and nsocio = '$idu'";
													$selecter = mysql_query($otrosql); 
													while($data = mysql_fetch_array($selecter)) {
														$vtotal = $data['total'];
													};

													$elsql="select c.periodo, p.descripcion, c.importe, c.vencimiento from cuotas c inner join plan_socio ps on ps.id = c.plan_socio_id 
													inner join planes p on p.id = ps.plan_id inner join socios s on s.id = ps.socio_id 
													where c.estado = 'Pendiente' and EXTRACT(YEAR_MONTH FROM c.vencimiento) <= EXTRACT(YEAR_MONTH FROM now()) and s.nsocio = '$idu'";
