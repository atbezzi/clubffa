	...
	$.ajax({
		...
		success: 
			function( data ) {
				...
				html += '<tr>'
							html += '<th class="success">Fecha</th>'
							html += '<th class="success">Importe</th>'
							html += '<th class="success">Acciones</th>'
						html += '</tr>';
				if(data.length > 0){
				  $.each(data, function(i,item){
					  html += '<tr>'
							...
							html += '<td><a href="buscarimprimirrepingreso.php?accion=imprimir&idp='+item.fecha+'&close=dor" target="_blank"><span class="glyphicon glyphicon-print" aria-hidden="true"></span></a></td>'
						html += '</tr>';
					});
				} 
