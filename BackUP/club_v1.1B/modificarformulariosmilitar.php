	...
	if ($accion =="modifica")
			{
				$resp = mysql_query("SELECT s.id as idsocio, s.nsocio as numsocio, s.nombre as nombre, s.apellido as apellido, 
					...
					s.categoria_id as categoria_id, s.estadoembarcadero as estadoembarcadero, m.id as idmilitar, m.socio_id as sociom, m.situacionmilitar as situacionmilitar, 
					m.grado_id as grado, m.fuerza_id as fuerza, m.escalafon_id as escalafon, m.promovidopor as promovidopor, m.localidad_id as localidadmilitar,
					m.serviciodesde as serviciodesde, m.serviciohasta as serviciohasta, p.id as provinciaid, p2.id as provinciaid2, p3.id as provinciaid3,
					p4.id as provinciaid4 FROM socios s
					INNER JOIN sociosm m ON s.id = m.socio_id 
					...
					INNER JOIN provincias p3 ON p3.id = l3.provincia_id INNER JOIN localidades l4 ON l4.id = m.localidad_id 
				...
				while($datos = mysql_fetch_array($resp)) {
					...
					$vsitumilitar = $datos['situacionmilitar'];
					$vgrado = $datos['grado'];
					$vfuerza = $datos['fuerza'];
					$vescalafon = $datos['escalafon'];
					$vpromovidopor = $datos['promovidopor'];
					$vprovmil = $datos['provinciaid4'];
					$vlocalidadmil = $datos['localidadmilitar'];
...
<div id="container">
	...
	<div class="row" style="max-width:100%; margin-left:0px; margin-right:0px;">
		...
		<div class="col-md-8 col-lg-8 col-xs-12 col-sm-12">
			<div class="main1">
				<div class="content">
					<h2><span class="glyphicon glyphicon-floppy-save"></span>&nbsp;Modificar socio militar</h2>
					<hr>
					<form id="frmcarga" name="frmcarga" method="post" action="modificarformulariosmilitar_post.php">
	...
	loadmodifica5(<?php echo $vprovmil.",".$vlocalidadmil; ?>);
	</script>
	<script type="text/javascript">
