								...
								<div class="content">
									<div class="table-responsive">
										...
										<table id="tableta" class="table table-striped table-bordered">
											...
											<th class="success text-center">Cobros</th>
											<th class="success text-center">Pagos</th>
										...
										<div class="alert alert-info" role="alert"><strong><label for="title" class="control-label"><u>Balance final:</u> </label><h1><label id="totalfinal" for="title" class="control-label"></label></strong></h1></div>
										</div>
									</div>
								</div>
							</div>
		...
		success: 
			function( data ) {
				for(var i=0;i<data.length; i++)
                           {
								if((i+1)==data.length){
									todojunto = todojunto+'["'+data[i].fecha+'",'+data[i].sumacobro+','+data[i].sumapago+']';
									todojunto1 = todojunto1+'["'+data[i].fecha+'",'+data[i].sumapago+','+data[i].sumacobro+']';	
									cobro = (parseFloat(cobro) + parseFloat(data[i].sumacobro)).toFixed(2) ;
                                    pago = (parseFloat(pago) + parseFloat(data[i].sumapago)).toFixed(2) ;									
								}else{
									todojunto = todojunto+'["'+data[i].fecha+'",'+data[i].sumacobro+','+data[i].sumapago+'],';
									todojunto1 = todojunto1+'["'+data[i].fecha+'",'+data[i].sumapago+','+data[i].sumacobro+'],';	
									cobro = (parseFloat(cobro) + parseFloat(data[i].sumacobro)).toFixed(2) ;
				...
				totalfinal = parseFloat(cobro) - parseFloat(pago);
                if(totalfinal > 0){
                    document.getElementById('totalfinal').innerHTML = " $"+totalfinal ;
                    document.getElementById('totalfinal').style.color = 'green';
                }else{
                    document.getElementById('totalfinal').innerHTML = " -$"+(totalfinal*(-1));
                    document.getElementById('totalfinal').style.color = 'red';
                }
				//tablaaaaaaaaaaaaaaa
				var html='';
				// si la consulta ajax devuelve datos
				html += '<tr>'
							html += '<th class="success text-center">Fehca</th>'
							html += '<th class="success text-center">Cobros</th>'
							html += '<th class="success text-center">Pagos</th>'
							html += '<th class="success text-center">Balance</th>'
						html += '</tr>';
				if(data.length > 0){
				  $.each(data, function(i,item){
					  html += '<tr>'
							html += '<td class="text-center">'+item.fecha+'</td>'
							html += '<td class="text-right">$ '+item.sumacobro+'</td>'
							html += '<td class="text-right">$ '+item.sumapago+'</td>'
                            totalmes = parseFloat(item.sumacobro) - parseFloat(item.sumapago);
                            if(totalmes>0){
                                html += '<td class="text-right"><font color="green">$'+totalmes+'</font></td>'
                            }else{
                                html += '<td class="text-right"><font color="red">-$'+(totalmes*-1)+'</font></td>'
                            } 
						html += '</tr>';
					});
				} 
			...
			todojunto1 = JSON.parse("["+todojunto1+"]");
			$("#containergraficos").highcharts().series[1].setData(todojunto1);
			},
