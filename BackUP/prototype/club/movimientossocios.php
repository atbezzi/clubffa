...
<div id="container">
	...
	<div class="row" style="max-width:100%; margin-left:0px; margin-right:0px;">
		...
		<div class="col-md-8 col-lg-8 col-xs-12 col-sm-12">
			<div class="main1">
				<div class="content">
					<h2><span class="glyphicon glyphicon-signal"></span>&nbsp;Movimientos de socios</h2>
					<br>
								...
								<div class="content">
									<div class="table-responsive">
										...
										<table id="tableta" class="table table-striped table-bordered">
											...
											<th class="success text-center">Altas</th>
											<th class="success text-center">Bajas</th>
											</tr>
...
<div class="col-md-1 col-lg-1 col-xs-12 col-sm-12">
	...
	$.ajax({
		...
		type    : 'POST',
		dataType: 'json',
		url: 'buscarmovimientossocios.php',
		success: 
			function( data ) {
				for(var i=0;i<data.length; i++)
                           {
								if((i+1)==data.length){
									todojunto = todojunto+'["'+data[i].fecha+'",'+data[i].cantidad+']';
								}else{
									todojunto = todojunto+'["'+data[i].fecha+'",'+data[i].cantidad+'],';
								}
				...
				html += '<tr>'
							...
							html += '<th class="success text-center">Altas</th>'
							html += '<th class="success text-center">Bajas</th>'
						html += '</tr>';
				if(data.length > 0){
				  $.each(data, function(i,item){
					  html += '<tr>'
							html += '<td class="text-center">'+item.fecha+'</td>'
							html += '<td class="text-center">'+item.alta+'</td>'
							html += '<td class="text-center">'+item.baja+'</td>'
						html += '</tr>';
					});
				} 
				...
				title: {
					text: 'Movimientos de socios por mes (ingresos y egresos)'
				},
				subtitle: {
					text: 'CFFAA CBA'
				},
