	...
	if ($accion =="evaluar")
			{
				...
				l4.descripcion as descripcion4, so.tipo as tiposol, so.observacion as observacion, m.situacionmilitar as situacionmilitar, m.grado_id as grado_id, m.fuerza_id as fuerza_id, 
				m.escalafon_id as escalafon, m.promovidopor as promovidopor, m.localidad_id as localidadmilitar, m.serviciodesde as serviciodesde, m.serviciohasta as serviciohasta, 
				z.id as idzona, z.descripcion as desczona, c.id as catid, c.descripcion as desccat FROM socios s INNER JOIN sociosm m ON s.id = m.socio_id 
				INNER JOIN solicitudes so ON s.id = so.socio_id INNER JOIN localidades l ON l.id = s.localidadnacimiento INNER JOIN localidades l2 ON l2.id = s.localidad_id 
				INNER JOIN localidades l3 ON l3.id = s.localidad_idcobrador INNER JOIN localidades l4 ON l4.id = m.localidad_id INNER JOIN zonas z ON z.id = s.zona_id 
				INNER JOIN categorias c ON c.id = s.categoria_id WHERE s.id = '$ids'");
				while($datos = mysql_fetch_array($resp)) {
					...
					$vsituacionmilitar = $datos['situacionmilitar'];
					$vgrado = $datos['grado_id'];
					$vfuerza = $datos['fuerza_id'];
					$vescalafon = $datos['escalafon'];
					$vpromovidopor = $datos['promovidopor'];
					$vlocalidadmilitar = $datos['descripcion4'];
					$vserviciodesde = $datos['serviciodesde'];
					$vserviciohasta = $datos['serviciohasta'];
...
<div id="container">
	...
	<div class="row" style="max-width:100%; margin-left:0px; margin-right:0px;">
		...
		<div class="col-md-8 col-lg-8 col-xs-12 col-sm-12">
			<div class="main1">
				<div class="content">
					<h2><span class="glyphicon glyphicon-list-alt"></span>&nbsp;Evaluar Solicitud Militar</h2>
					<br><br><br>
					<form id="frmcarga" name="frmcarga" method="post" action="evaluarsolicitudes_post.php">
						<div class="table-responsive">
						<h3>Tipo de Afiliado: <span class="label label-primary"><?php echo $vtipo;?></span></h3><hr>
						<h3>N° Afiliado: <span class="label label-primary"><?php echo $vnumsocio;?></span></h3><hr>
						...
						<h3>Telefono: <span class="label label-primary"><?php echo $vtelefono;?></span></h3><hr>
						...
						<h3>Categoria: <span class="label label-primary"><?php echo $vcategorias;?></span></h3><hr>
						<h3>Embarcadero: <span class="label label-primary"><?php echo $vembarca;?></span></h3><hr>
						<h3>Situacion Militar: <span class="label label-primary"><?php echo $vsituacionmilitar;?></span></h3><hr>
						<h3>Grado: <span class="label label-primary"><?php echo $vgrado;?></span></h3><hr>
						<h3>Fuerza: <span class="label label-primary"><?php echo $vfuerza;?></span></h3><hr>
						<h3>Escalafon: <span class="label label-primary"><?php echo $vescalafon;?></span></h3><hr>
						<h3>Promovido por: <span class="label label-primary"><?php echo $vpromovidopor;?></span></h3><hr>
						<h3>Localidad Militar: <span class="label label-primary"><?php echo $vlocalidadmilitar;?></span></h3><hr>
						<h3>Servicio Desde: <span class="label label-primary"><?php echo $vserviciodesde;?></span></h3><hr>
						<h3>Servicio Hasta: <span class="label label-primary"><?php echo $vserviciohasta;?></span></h3>
						<?php
							if ($vtiposol == 'Egreso'){
									?>
									<hr>
									<h3>Observacion: <span class="label label-primary"><?php echo $vobserva;?></span></h3>
									<?php
								} else{
