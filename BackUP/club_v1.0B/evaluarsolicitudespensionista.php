	...
	if ($accion =="evaluar")
			{
				...
				l4.descripcion as descripcion4, so.tipo as tiposol, so.observacion as observacion, p.grado_id as grado_id, p.fuerza_id as fuerza_id, 
				p.escalafon_id as escalafon, p.promovidopor as promovidopor, p.localidad_id as localidadmilitar, p.serviciodesde as serviciodesde, p.serviciohasta as serviciohasta, 
				z.id as idzona, z.descripcion as desczona, c.id as catid, c.descripcion as desccat FROM socios s INNER JOIN sociosp p ON s.id = p.socio_id INNER JOIN solicitudes so ON s.id = so.socio_id 
				INNER JOIN localidades l ON l.id = s.localidadnacimiento INNER JOIN localidades l2 ON l2.id = s.localidad_id 
				INNER JOIN localidades l3 ON l3.id = s.localidad_idcobrador INNER JOIN localidades l4 ON l4.id = p.localidad_id 
				INNER JOIN zonas z ON z.id = s.zona_id INNER JOIN categorias c ON c.id = s.categoria_id WHERE s.id = '$ids'");
...
<div id="container">
	...
	<div class="row" style="max-width:100%; margin-left:0px; margin-right:0px;">
		...
		<div class="col-md-8 col-lg-8 col-xs-12 col-sm-12">
			<div class="main1">
				<div class="content">
					<h2><span class="glyphicon glyphicon-list-alt"></span>&nbsp;Evaluar Solicitud Pensionista</h2>
					<br><br><br>
					<form id="frmcarga" name="frmcarga" method="post" action="evaluarsolicitudes_post.php">
						...
						<h3>Nombre Militar: <span class="label label-primary"><?php echo $vpromovidopor;?></span></h3><hr>
						...
						<h3>Servicio Hasta: <span class="label label-primary"><?php echo $vserviciohasta;?></span></h3><hr>
						<?php
							if ($vtiposol == 'Egreso'){
									?>
									<hr>
