...
include_once 'inc/conexion.php';
	...
	l3.id as idlocalidad3, l3.descripcion as descripcion3, z.id as idzona, z.descripcion as desczona, c.id as catid, c.descripcion as desccat FROM socios s 
	INNER JOIN localidades l ON l.id = s.localidadnacimiento INNER JOIN localidades l2 ON l2.id = s.localidad_id INNER JOIN localidades l3 ON l3.id = s.localidad_idcobrador 
	INNER JOIN zonas z ON z.id = s.zona_id INNER JOIN categorias c ON c.id = s.categoria_id WHERE s.id = '$idu'");
	...
	if ($total == 1)
	{
		...
		while($datos = mysql_fetch_array($resp)) {
			...
			$vzona_id = $datos['desczona'];
			$vlibro = $datos['libro'];
			$vacta = $datos['acta'];
			$vcategoria_id = $datos['desccat'];
...
'domiciliocobrador' => $vdomiciliocobrador, 'barriocobrador' => $vbarriocobrador, 'descripcion3' => $vlocalidad_idcobrador, 'desczona' => $vzona_id, 'libro' => $vlibro, 
'acta' => $vacta, 'desccat' => $vcategoria_id, 'estadoembarcadero' => $vestadoembarcadero, 'estado' => $vestado, 'encontro' => $encontro);
