									...
									<div class="table-responsive">
										<table id="tableta" class="table table-striped table-bordered">
											...
											<?php 
												...
												$elsql="select c.id as id, c.periodo as periodo, p.descripcion, c.importe, DATE_FORMAT(c.vencimiento,'%d/%m/%Y') as vencimiento 
												from cuotas c inner join plan_socio ps on ps.id = c.plan_socio_id inner join socios s on s.id = ps.socio_id 
												inner join planes p on p.id = ps.plan_id where s.nsocio = '$idu' and c.estado = 'Pendiente' order by c.periodo asc";
												$select = mysql_query($elsql); 
												// echo $elsql;
												while($datos = mysql_fetch_array($select)) {
											?>
												<tr id="tablita">
												<td id="tdcheck"><input type="checkbox" onChange="calculartotal();" value="<?php echo $datos['id'];?>" id="check" name="check"></td>
												<td id="tdperiodo"><input type="text" name="periodo" class="form-control" id="periodo" value="<?php echo $datos['periodo'];?>" disabled="disabled"></td>
												<td id="tdcuota"><input type="text" name="descripcion" class="form-control" id="descripcion" value="<?php echo $datos['descripcion'];?>" disabled="disabled"></td>
												<td id="tdimporte"><input type="text" name="importe" class="form-control" id="importe" value="<?php echo $datos['importe'];?>" disabled="disabled"></td>
												<td id="tddetalle"><input type="text" name="vencimiento" class="form-control" id="vencimiento" value="<?php echo $datos['vencimiento'];?>" disabled="disabled"></td>
												</tr>
												<?php
												} ;
											?>
		...
		function calculartotal(){
			...
			for (var i=1;i < document.getElementById('tableta').rows.length; i++){
				if(document.getElementById('tableta').rows[i].cells[0].children[0].checked){
					sum = document.getElementById('tableta').rows[i].cells[3].children[0].value;
	...
	function grabaTodoTabla(ID){
		...
		TABLA.each(function(){
			...
			var IDS = $(this).find("input[id*='check']").val(),
				PER = $(this).find("input[id*='periodo']").val(),
				CUOT = $(this).find("input[id*='descripcion']").val(),
				DET = $(this).find("input[id*='vencimiento']").val(),
			...
			if(CON != ''){
				item ["id"] = IDS;
				item ["periodo"] = PER;
				item ["descripcion"] = CUOT;
				item ['vencimiento'] = DET;
				item ['importe'] = IMP;
