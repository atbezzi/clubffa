			...
			<ul class="nav pull-right">
			  <li class="dropdown">
				...
				<div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
					...
					<a href="secret.php">Administracion de Socio</a><br /><br />
                    <a href="secret.php">Administracion de Embarcacion</a>   
					</form> 
				</div>
			  </li>
...
<div class="container hero-unit" id="vasiosi" style="display:;">
	...
	<div class="col-sm-6 text-center">
		...
		<li>
		 <label for="localidad">Provincia - Localidad</label>
		 ...
		 $sql = "select provincia, localidad from provincias inner join localidades on provincias.id = localidades.id_provincia";
		 $query = "SELECT * FROM provincias AND localidades";
		  $result = mysql_query($sql);
		  $numero = 0;
		  while($row = mysql_fetch_array($result))
		  { 
				if ($localidad_v ==  $row['provincia']." - ".$row['localidad'])
				{
				?>
			<option selected value="<?php echo $row['provincia']." - ".$row['localidad'];?>"><?php echo $row['provincia']." - ".$row['localidad'];?></option>
		<?php 
			}
			else
			{
		 ?>
				<option value="<?php echo $row['provincia']." - ".$row['localidad'];?>"><?php echo $row['provincia']." - ".$row['localidad'];?></option>
		<?php 		}
		  }
		?>
		 </select>
